...
test('game state initializes from local storage and normalizes arrays', async () => {
  ...
  localStorage.setItem('liminal_save', JSON.stringify({
    intro: { completed: true, version: '2', completedAt: '2026-03-15T10:00:00.000Z' },
    collectedLore: ['1', 1, 2],
  ...
  assert.equal(state.state.intro.version, 2);
...
test('game state exposes intro completion helpers', async () => {
  ...
  assert.equal(state.isIntroCompleted(2), false);
  const intro = await state.markIntroCompleted(2, '2026-03-15T12:30:00.000Z');
  assert.deepEqual(intro, {
    completed: true,
    version: 2,
    completedAt: '2026-03-15T12:30:00.000Z'
  });
  assert.equal(state.isIntroCompleted(2), true);
});
